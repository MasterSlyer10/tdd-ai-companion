(function () {
  ...
  let isRequestCancelled = false; // Flag to track if the current request is cancelled  let activePromptId = null; // Track the ID of the currently active prompt  // DOM Elements
  ...
  function init() {
    ...
    activePromptId = null;    // Load token count from state
    loadTokenCountFromState();    // Initialize and display token count
    initializeTokenDisplay(); // Create the display element and place it    updateTokenDisplay(); // Set initial text and color
    
    // Indexing status request removed - debug UI cleanup
